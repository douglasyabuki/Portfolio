...
export default function AboutBoard({ infoList }: Props) {
  ...
  return (
    <div className="container relative block h-auto w-full lg:flex lg:py-20">
      <div className={`${currentSectionId !== null ? `sticky top-32 z-30 w-fit h-0 -translate-x-6 lg:relative lg:top-0 lg:-translate-x-0`: `-translate-x-0 block h-auto w-auto text-center`} space-y-4 lg:space-y-0 lg:h-auto lg:mr-6 lg:py-12 transition-all duration-300`}>
        {infoList.map((item) => (
          <AboutBoardSelector
            id={item.sectionId}
